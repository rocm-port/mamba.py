...
class ResidualBlock(nn.Module):
    ...
    def step(self, x):
        ...
        output = self.mixer.step(self.norm(x))
        output = output + x
        return output
...
class MambaBlock(nn.Module):
    ...
    def reset_state(self):
        self.cache = torch.zeros(1, self.d_inner, self.d_conv-1, device=next(self.parameters()).device)
        self.h = torch.zeros(1, self.d_inner, self.d_state, device=next(self.parameters()).device)
    ...
    def step(self, x):
        ...
        x = self.conv1d(torch.cat([self.cache, x_cache], dim=2))[:, :, self.d_conv-1] # (B, ED)
        ...
        self.cache = torch.cat([self.cache[:, :, 1:], x_cache], dim=2) # (B, ED, d_conv-1)
