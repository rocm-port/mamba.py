...
import mamba_state as mamba_impl
...
@dataclass
class MambaLMStateConfig(mamba_impl.MambaConfig):
...
def from_pretrained(name: str):
    ...
    config = MambaLMStateConfig(d_model=config_data['d_model'], n_layers=config_data['n_layer'], vocab_size=config_data['vocab_size'])

    model = MambaLMState(config)
...
class MambaLMState(nn.Module):
    def __init__(self, lm_config: MambaLMStateConfig):
        super().__init__()
    ...
    def init_runs(self, tokenizer, prompt: str, num_tokens: int = 50, sample: bool = True, top_k: int = 40):
        input_ids = tokenizer(prompt, return_tensors='pt').input_ids.to(next(self.parameters()).device) # (1, num_tokens)
        h = torch.zeros(len(self.mamba.layers), input_ids.size(0), self.config.d_inner, self.config.d_state, device=input_ids.device) # (B, ED, N)
        caches = torch.zeros(len(self.mamba.layers), input_ids.size(0), self.config.d_inner, self.config.d_state, device=input_ids.device) # (B, ED, N)
    ...
    def generate(self, tokenizer, prompt: str, num_tokens: int = 50, sample: bool = True, top_k: int = 40):
        ...
        input_ids, caches = self.init_runs(tokenizer, prompt, num_tokens, sample, top_k)

        for i in range(input_ids.size(1) + num_tokens - 1):
            with torch.no_grad():
                # forward the new output, get new cache
                (next_token_logits, caches) = self.forward((input_ids[:, i], caches)) # (1, vocab_size), caches
